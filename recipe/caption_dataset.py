...
class CaptionRLHFDataset(Dataset):
    """
    ...
    def __init__(
        ...
        if self.tool_config_path:
            try:
                ...
                logger.warning(
                    "Failed to initialize tools from %s: %s", self.tool_config_path, e
                )
                self.tool_schemas = None
        ...
        self.num_workers = (
            min(self.num_workers, os.cpu_count())
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        if image_key in doc and doc[image_key]:
                            images = [
                                process_image(
                                    image, image_patch_size=self.image_patch_size
                                )
    ...
    def __getitem__(self, item):
        ...
        # Parse JSON string fields back to dict/list
        if isinstance(row_dict.get(self.prompt_key), str):
            row_dict[self.prompt_key] = json.loads(row_dict[self.prompt_key])
        ...
        if self.image_key in row_dict and isinstance(row_dict[self.image_key], str):
            image_paths = json.loads(row_dict[self.image_key])
            
            # Get max_image_token from row_dict
            max_image_token = row_dict.get("max_image_token", 4096)
            
            # Calculate resize parameters
            factor = self.image_patch_size * 2
            min_pixels = 256 * self.image_patch_size * self.image_patch_size
            max_pixels = max_image_token * self.image_patch_size * self.image_patch_size
            ...
            for img_path in image_paths:
                ...
                new_height, new_width = _smart_resize(
                    height=height,
                    width=width,
                    factor=factor,
            ...
            row_dict[self.image_key] = resized_images

        need_tools_kwargs: bool = row_dict.get("extra_info", {}).get(
